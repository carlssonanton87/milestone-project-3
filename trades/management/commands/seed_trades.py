...
class Command(BaseCommand):
    ...
    def handle(self, *args, **kwargs):
        ...
        outcomes = (['win'] * win_trades) + (['loss'] * (closed_trades - win_trades)) + (['open'] * open_trades)
        ...
        for i in range(total_trades):
            ...
            if outcome == 'open':
                ...
                exit_price = entry_price * (1 + return_pct / 100) if outcome == 'win' else entry_price * (1 - return_pct / 100)
        ...
        self.stdout.write(self.style.SUCCESS(f'Successfully seeded {total_trades} trades for user: {user.username}'))
